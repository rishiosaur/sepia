...
impl<'a> Lexer<'a> {
    ...
    pub fn integer(&mut self) -> Option<Token> {
        ...
        match num_type {
            NumberTypes::Integer => match num.parse::<usize>() {
                Ok(n) => Some(Token {
                ...
                _ => {
                    panic!(
                        "[{:?}] LEXER ERROR: Could not parse integer.",
                        current_position
                    );
                }
            },
            NumberTypes::Float => match num.parse::<f64>() {
                Ok(n) => Some(Token {
                ...
                _ => {
                    panic!(
                        "[{:?}] LEXER ERROR: Could not parse float.",
                        current_position
                    );
                }
            },
        ...
        match current_character.unwrap() {
            ...
            '/' => {
                match peek_character {
                    // Some('*') => self.double_char_tok(TokenType::OpenBlock),
                    None | Some(' ') | Some('\t') | Some('\r') => {
                        self.single_char_tok(TokenType::Slash)
                    }
                    _ => panic!("[{:?}] LEXER ERROR: Undefined token", self.literal_position),
                }
            }
            ...
            '=' => match peek_character {
                ...
                None | Some(' ') | Some('\t') | Some('\r') => {
                    self.single_char_tok(TokenType::Assign)
                }
                _ => panic!("[{:?}] LEXER ERROR: Undefined token", self.literal_position),
            },
            '!' => match peek_character {
